...
bool BinarySearchTree<T>::isBalanced(TreeNode<T>* tNode)
{
  //DO THIS
	...
	isBalanced(tNode->getLeft());
	isBalanced(tNode->getRight());
...
void BinarySearchTree<T>::minimizeComplete(T** items, int first, int last)
{
   ...
   if (first <= last)
   {
		...
		if (first < last)
		{
			...
			double k_left = log2(mid);          //log base 2 of the number of items to the left of mid (including mid)
			double int_k_left = (int) k_left;              //same as above but rounded
			double k_right = log2(last - mid);
			...
			while (fabs(k_left - int_k_left) > TOL && fabs(k_right - int_k_right) > TOL)
			{
				...
				k_left = log2(mid);          //log base 2 of the number of items to the left of mid (including mid)
				int_k_left = (int) k_left;              //same as above but rounded
				k_right = log2(last - mid);
		...
		this->insert(items[mid]);
		cout << "mid: " << mid << endl << endl << "first: " << first << endl << endl << "last: " << last << endl << endl;
		...
		minimizeComplete(items, first + 1, mid - 1);
